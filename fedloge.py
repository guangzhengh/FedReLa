...
# FedAvg weights: w_locals[i] is client i when training all clients in order 0..N-1.
...
if __name__ == '__main__':
    ...
    for rnd in tqdm(range(rndstart, args.rounds)):
        ...
        # Local training: all clients if fedavg_align_weights_with_client (same as fed_etf.py); else subset idxs_users.
        g_head.train()
        local_losses = []
        if fedavg_align_weights_with_client:
            client_order = range(args.num_users)
        else:
            client_order = idxs_users
        ...
        patience = 10#int(getattr(args, "early_stop_after_relabel_patience", 0) or 0)
        ...
        # avg local train loss across trained clients (weighted by local data size)
        if local_losses:
        ...
        logger.log_round(
            RoundLogRow(
                ...
                num_users_trained=int(len(client_order)),
