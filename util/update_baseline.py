...
class LocalUpdate(object):
    ...
    def update_weights_gaux(self, net, g_head, g_aux, l_head, epoch, args, mu=1, lr=None, loss_switch=None, net_id=0, rnd=0):
        net.train()
        relabel_lr_scale = 0.1 if rnd >= args.relabel_start else 1.0
        cur_lr = self.args.lr * relabel_lr_scale
        # train and update
        optimizer_g_backbone = torch.optim.SGD(list(net.parameters()), lr=cur_lr, momentum=self.args.momentum)
        optimizer_g_aux = torch.optim.SGD(g_aux.parameters(), lr=cur_lr, momentum=self.args.momentum)
        ...
        optimizer_l_head = torch.optim.SGD(l_head.parameters(), lr=cur_lr, momentum=self.args.momentum)
        # Define optimizer
        ...
        if rnd >= args.relabel_cal:
            is_store_round = (rnd == args.relabel_start) or (getattr(args, "relabel_period", 0) and ((rnd - args.relabel_start) % args.relabel_period == 0))
            read = 0 if is_store_round else 1
            store = 1 if is_store_round else 0
            relabel_info = self.relabel_local_data(
                copy.deepcopy(g_head),
                copy.deepcopy(net),
                net_id,
                args,
                relabel_thre=args.thre,
        ...
        return net.state_dict(), g_aux, l_head, sum(epoch_loss) / len(epoch_loss), fflag
    ...
    def update_weights_etf(self, net, g_head, g_aux, l_head, epoch, args, mu=1, lr=None, loss_switch=None, net_id=0, rnd=0):
        net.train()
        relabel_lr_scale = 0.1 if rnd >= args.relabel_start else 1.0
        cur_lr = self.args.lr * relabel_lr_scale
        # train and update
        optimizer_g_backbone = torch.optim.SGD([{"params": net.parameters()},
                                {"params": g_head.parameters()}], lr=cur_lr, momentum=self.args.momentum)
        optimizer_g_aux = torch.optim.SGD(g_aux.parameters(), lr=cur_lr, momentum=self.args.momentum)
        # optimizer_g_aux = torch.optim.SGD([
        #                     {'params': g_aux.weight, 'weight_decay': 1e-1},  # Use weight_decay for weights
        #                     {'params': g_aux.bias, 'weight_decay': 0}  # Do not use weight_decay for bias
...
def globaltest(net, g_head, test_dataset, args, dataset_class=None, head_switch=True):
    ...
    total_class_label = [0 for i in range(args.num_classes)]
    predict_true_class = [0 for i in range(args.num_classes)]
    # Optional confusion matrices (test-set): CIFAR-10 full 10x10, CIFAR-100 grouped 3x3.
    collect_conf = bool(getattr(args, "save_test_confusion", False))
    conf_full = None
    conf_group = None
    if collect_conf and args.num_classes <= 10:
        conf_full = np.zeros((args.num_classes, args.num_classes), dtype=np.int64)
    if collect_conf and args.num_classes > 10:
        conf_group = np.zeros((3, 3), dtype=np.int64)  # head/middle/tail

    head_set = set(three_shot_dict["head"])
    middle_set = set(three_shot_dict["middle"])
            ...
            for i in range(len(labels)):
                ...
                if collect_conf:
                    ...
                    if conf_full is not None:
                        conf_full[t, p] += 1
                    elif conf_group is not None:
                        conf_group[group_id(t), group_id(p)] += 1
    ...
    acc_3shot_global["tail"] = correct_3shot["tail"] / \
        (total_3shot["tail"] + 1e-10)
    if collect_conf:
        labels_out = list(range(args.num_classes)) if conf_full is not None else ["head", "middle", "tail"]
        conf_out = conf_full.tolist() if conf_full is not None else conf_group.tolist()
        return acc, acc_3shot_global, conf_out, labels_out
    return acc, acc_3shot_global
...
def globaltest_etf(net, g_head, test_dataset, args, dataset_class=None, head_switch=True):
    ...
    if collect_conf and args.num_classes <= 10:
        conf_full = np.zeros((args.num_classes, args.num_classes), dtype=np.int64)
    if collect_conf and args.num_classes > 10:
        conf_group = np.zeros((3, 3), dtype=np.int64)
