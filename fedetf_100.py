...
if __name__ == '__main__':
    ...
    for rnd in tqdm(range(rndstart, args.rounds)):
        ...
        for client_id in range(args.num_users):
            ...
            w_local, g_aux_temp, l_heads[client_id], loss_local, _relabel_fflag = local.update_weights_etf(net=copy.deepcopy(model).to(args.device), g_head = copy.deepcopy(g_head).to(args.device), g_aux = copy.deepcopy(g_aux).to(args.device), l_head = l_heads[client_id], epoch=args.local_ep, loss_switch = loss_switch, args=args, net_id=client_id, rnd=rnd)
